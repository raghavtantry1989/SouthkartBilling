...
public class BillingProvider extends ContentProvider {
    ...
    static {
        ...
        sUriMatcher.addURI(BillingContract.CONTENT_AUTHORITY,BillingContract.PATH_PRODUCTS,PRODUCT);

        // Matcher for "content://com.southkart.billing/products/#" 201
        sUriMatcher.addURI(BillingContract.CONTENT_AUTHORITY,BillingContract.PATH_PRODUCTS+"/#",PRODUCT_ID);
    }

    // SqLite Database reference used by BillingProvider
    private BillingDbHelper mDbHelper;

    @Override
    public boolean onCreate() {
        mDbHelper = new BillingDbHelper(getContext());
        return false;
    }
    ...
    private Uri insertSupplier(Uri uri, ContentValues values) {
        ...
        if(id == -1){
            Log.e("BillingProvider", "Failed to insert row for "+uri);
        }
